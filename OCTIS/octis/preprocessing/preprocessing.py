...
class Preprocessing:
    def __init__(
        ...
        save_original_indexes=True, remove_stopwords_spacy: bool = True, encoding='utf-8'):
        """
        init Preprocessing
    ...
    def preprocess_dataset(self, documents_path, labels_path=None, multilabel=False):
        ...
        """
        docs = [line.strip() for line in open(documents_path, 'r',encoding=self.encoding).readlines()]
        ...
        # Step 2: Remove single-letter words and create vocabulary
        # Step 2: Remove single-letter words and join back into strings
        docs = [" ".join([word for word in doc.split() if len(word) > 1]) for doc in docs]
        vocabulary = self.filter_words(docs)
        print("vocab created. vocab size: ",len(vocabulary))
        final_docs, final_labels, document_indexes = [], [], []
        if labels_path is not None:
            if multilabel:
                labels = [
                    line.strip().split()
                    for line in open(labels_path, 'r', encoding=self.encoding).readlines()]
            else:
                labels = [
                    line.strip()
                    for line in open(labels_path, 'r', encoding=self.encoding).readlines()]
